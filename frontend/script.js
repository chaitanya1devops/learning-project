...
const devOpsData = {
    ...
    "Kubernetes Commands": [
        { q: "kubectl version", a: "Show client + server K8s version." },
        { q: "kubectl cluster-info", a: "See cluster master & DNS info." },
        { q: "kubectl get all", a: "List all resources (pods, svc, deployments, etc.) in default namespace." },
        { q: "kubectl config get-contexts", a: "List all contexts (clusters)." },
        { q: "kubectl config use-context dev", a: "Switch to another cluster." },
        { q: "kubectl config current-context", a: "Show which cluster you are using." },
        { q: "kubectl get namespaces", a: "List namespaces." },
        { q: "kubectl create namespace dev", a: "Create namespace." },
        { q: "kubectl delete namespace dev", a: "Delete namespace." },
        { q: "kubectl config set-context --current --namespace=dev", a: "Set default namespace." },
        { q: "kubectl get pods", a: "List pods." },
        { q: "kubectl get pods -o wide", a: "Show pod IP, node, etc." },
        { q: "kubectl describe pod pod-name", a: "Detailed pod info & events." },
        { q: "kubectl logs pod-name", a: "View logs." },
        { q: "kubectl logs -f pod-name", a: "Follow live logs." },
        { q: "kubectl exec -it pod-name -- bash", a: "Enter pod terminal." },
        { q: "kubectl delete pod pod-name", a: "Delete a pod." },
        { q: "kubectl get deployments", a: "List deployments." },
        { q: "kubectl create deployment web --image=nginx", a: "Create deployment." },
        { q: "kubectl scale deployment web --replicas=5", a: "Scale number of pods." },
        { q: "kubectl rollout status deployment/web", a: "Check deployment rollout status." },
        { q: "kubectl rollout undo deployment/web", a: "Rollback to previous version." },
        { q: "kubectl delete deployment web", a: "Delete deployment." },
        { q: "kubectl get rs", a: "List ReplicaSets." },
        { q: "kubectl describe rs rs-name", a: "ReplicaSet details." },
        { q: "kubectl get svc", a: "List services." },
        { q: "kubectl expose deployment web --type=NodePort --port=80", a: "Expose deployment as a service." },
        { q: "kubectl describe svc web", a: "Service details." },
        { q: "kubectl get svc -o wide", a: "See service cluster IP & ports." },
        { q: "kubectl create configmap app-config --from-literal=env=prod", a: "Create ConfigMap." },
        { q: "kubectl create configmap myconfig --from-file=config.properties", a: "Create from file." },
        { q: "kubectl get configmaps", a: "List ConfigMaps." },
        { q: "kubectl describe configmap app-config", a: "View config details." },
        { q: "kubectl create secret generic db-secret --from-literal=password=1234", a: "Create secret." },
        { q: "kubectl create secret generic tls-secret --from-file=server.crt --from-file=server.key", a: "Create TLS secret." },
        { q: "kubectl get secrets", a: "List secrets." },
        { q: "kubectl describe secret db-secret", a: "Describe secret." },
        { q: "kubectl apply -f deployment.yaml", a: "Apply or update resource." },
        { q: "kubectl delete -f deployment.yaml", a: "Delete resource." },
        { q: "kubectl edit deployment web", a: "Edit resource live in editor." },
        { q: "kubectl get nodes", a: "List nodes." },
        { q: "kubectl get nodes -o wide", a: "Node details (OS, internal IP)." },
        { q: "kubectl describe node node1", a: "Node details (taints, capacity)." },
        { q: "kubectl drain node1 --ignore-daemonsets", a: "Drain node safely." },
        { q: "kubectl cordon node1", a: "Mark node unschedulable." },
        { q: "kubectl uncordon node1", a: "Mark node schedulable." },
        { q: "kubectl top pods", a: "CPU & memory usage of pods (Metrics server required)." },
        { q: "kubectl top nodes", a: "CPU & memory usage of nodes (Metrics server required)." },
        { q: "kubectl taint nodes node1 key=value:NoSchedule", a: "Add taint to node. Used for dedicate nodes, restrict workloads, isolation." },
        { q: "kubectl taint nodes node1 key=value:NoSchedule-", a: "Remove taint." },
        { q: "kubectl label pod web app=frontend", a: "Add label." },
        { q: "kubectl get pods -l app=frontend", a: "Filter with label." },
        { q: "kubectl label pod web app-", a: "Remove label." },
        { q: "kubectl port-forward pod/mypod 8080:80", a: "Access pod locally. Common for debugging APIs." },
        { q: "kubectl get ingress", a: "List ingress rules." },
        { q: "kubectl describe ingress my-ingress", a: "Ingress details." },
        { q: "kubectl get statefulsets", a: "List StatefulSets." },
        { q: "kubectl describe statefulset mysql", a: "Stateful app details. Used for databases, Kafka, ElasticSearch." },
        { q: "kubectl get daemonsets", a: "List DaemonSets. Used for logging agents, monitoring agents." },
        { q: "kubectl create job myjob --image=busybox", a: "Create Job." },
        { q: "kubectl create cronjob backup --image=busybox --schedule=\"*/5 * * * *\"", a: "Create CronJob." },
        { q: "kubectl get jobs", a: "List jobs." },
        { q: "kubectl get cronjobs", a: "List CronJobs." },
        { q: "kubectl describe pod web", a: "Check events and status." },
        { q: "kubectl logs pod --previous", a: "Check logs of crashed container." },
        { q: "kubectl exec -it web -- sh", a: "Debug inside pod." },
        { q: "kubectl get pod web -o yaml", a: "View complete pod spec." },
        { q: "kubectl get events", a: "Cluster-wide events." },
        { q: "kubectl get endpoints", a: "Check service endpoints." },
        { q: "kubectl get componentstatus", a: "Master component status (older K8s)." },
        { q: "kubectl get networkpolicies", a: "Check network restrictions." },
        { q: "kubectl get netpol", a: "List policies." },
        { q: "kubectl describe netpol", a: "Network policy details. Used for pod-to-pod communication restriction, zero-trust networking." },
        { q: "kubectl get pv", a: "List persistent volumes." },
        { q: "kubectl get pvc", a: "List persistent volume claims." },
        { q: "kubectl describe pv pv-name", a: "PV details." },
        { q: "kubectl describe pvc pvc-name", a: "PVC details." },
        { q: "kubectl get serviceaccounts", a: "List service accounts." },
        { q: "kubectl create serviceaccount dev-sa", a: "Create SA." },
        { q: "kubectl get clusterrole", a: "List cluster roles." },
        { q: "kubectl get clusterrolebinding", a: "List bindings." },
        { q: "kubectl delete all --all", a: "Delete pods, svc, deployments in namespace." },
        { q: "kubectl delete namespace dev", a: "Delete entire namespace." },
        { q: "kubectl get po", a: "Short for pods." },
        { q: "kubectl get deploy", a: "Short for deployments." },
        { q: "kubectl get ing", a: "Short for ingress." },
        { q: "kubectl get cm", a: "Short for configmap." },
        { q: "kubectl get no", a: "Short for nodes." },
        { q: "kubectl apply -f app.yaml --dry-run=client", a: "Check if YAML is valid without applying." },
        { q: "kubectl get nodes -o json | jq '.items[].status.conditions'", a: "Detailed node conditions." },
        { q: "kubectl get pod web -o yaml > pod.yaml", a: "Export pod definition. Useful for reproducing or modifying pods." },
        { q: "kubectl proxy", a: "Start API server proxy for debugging." },
        { q: "kubectl exec -it pod --container app -- sh", a: "Exec into a specific container inside a multi-container pod." },
        { q: "kubectl logs pod -c app", a: "View logs of a specific container." },
        { q: "kubectl describe pod pod | grep -i error", a: "Search for errors in pod events." },
        { q: "kubectl debug podname -it --image=busybox", a: "Attach a temporary debug container (K8s 1.23+)." },
        { q: "kubectl debug node/node1 -it --image=busybox", a: "Debug an entire node (K8s 1.23+). Used for CrashLoopBackOff, ImagePullBackOff, node-level debugging." },
        { q: "kubectl get pod pod -o jsonpath='{.status.containerStatuses[].state.waiting.reason}'", a: "Get reason for failed pod. Common: ImagePullBackOff, ErrImagePull, CrashLoopBackOff." }
    ],
    "OpenShift": [
    ...
    "Scenario-Based Kubernetes": [
        { q: "Scenario 1: Pod CrashLoopBackOff - How to troubleshoot?", a: "Troubleshoot using:\n- kubectl describe pod <pod>\n- kubectl logs <pod> --previous\nCheck for:\n- App crash\n- Wrong image tag or entrypoint\n- Misconfigured probes\n- Missing configs or secrets" },
        { q: "Scenario 2: Nodes are Not Ready - How to debug?", a: "Check with:\n- kubectl describe node <node>\n- systemctl status kubelet\nLook for disk issues, network problems, OOM, or cert expiration." },
        { q: "Scenario 3: Rolling Update Failed - What causes it?", a: "Check pod events with:\n- kubectl describe pod <pod>\nCauses include unschedulable resource requests, node affinity, or missing PVCs." },
        { q: "Scenario 4: Service Not Accessible - How to fix?", a: "Use:\n- kubectl get svc <name> -o wide\n- kubectl get endpoints <svc>\nCheck if pod labels match selector, correct ports used, and test via 'kubectl exec curl'." },
        { q: "Scenario 5: PVC Not Bound - How to resolve?", a: "Check with:\n- kubectl get pvc,pv\nEnsure StorageClass, accessModes, and size match. Fix or provision dynamic PV." },
        { q: "Scenario 6: External Config Injection - Implementation approach?", a: "Use ConfigMap and Secret via environment or volume mount.\nValidate existence and permissions. Example: envFrom -> configMapRef or secretRef." },
        { q: "Scenario 7: HPA Not Scaling - Why?", a: "Check metrics:\n- kubectl top pod\n- kubectl describe hpa\nEnsure metrics-server is installed, and CPU requests are set." },
        { q: "Scenario 8: Canary Deployment - How to implement?", a: "Use multiple Deployments with traffic split via labels.\nOr use Istio/NGINX Ingress with route weight control." },
        { q: "Scenario 9: Cluster Upgrade - Best practices?", a: "Backup etcd and manifests.\nDrain nodes:\n- kubectl drain <node> --ignore-daemonsets\nUpgrade kubeadm, kubelet, and kubectl in order." },
        { q: "Scenario 10: Secret Rotation - How to manage?", a: "Rotate secrets in Vault or Kubernetes. Restart pods to apply.\nUse external secrets operator or automation pipeline." },
        { q: "Scenario 11: Stuck Finalizers - How to fix?", a: "Patch the resource to remove finalizers using kubectl patch.\nUsed when namespaces or CRDs are stuck in Terminating state." },
        { q: "Scenario 12: DaemonSet Missing on Node - Cause?", a: "Check for taints on the node. DaemonSet needs tolerations to be scheduled." },
        { q: "Scenario 13: CPU Usage > Request But Not Throttled - Why?", a: "Throttling only happens if 'limits' are defined. Without it, CPU can burst up to node capacity." },
        { q: "Scenario 14: NetworkPolicy Blocking - How to debug?", a: "Pods in same namespace may be blocked by NetworkPolicy.\nCheck kubectl get networkpolicy and ensure ingress/egress rules are applied." },
        { q: "Scenario 15: Readiness vs Liveness Probe - Difference?", a: "Liveness succeeds = pod running.\nReadiness fails = pod removed from service endpoints but not restarted." },
        { q: "Scenario 16: Autoscaler Not Scaling - Troubleshooting?", a: "Pending pods + autoscaler = check taints, labels, max nodes, and IAM roles." },
        { q: "Scenario 17: External API Timeout - How to diagnose?", a: "Test from pod using curl. Check egress restrictions, NAT gateway, DNS resolution, or NetworkPolicy." },
        { q: "Scenario 18: Too Many ReplicaSets - Solution?", a: "Helm or Deployment may leave old ReplicaSets.\nSet 'revisionHistoryLimit' to limit history." },
        { q: "Scenario 19: GPU Pod Not Scheduling - Fix?", a: "Check if NVIDIA device plugin is installed and GPU available on node. Use correct resource key:\nvidia.com/gpu." },
        { q: "Scenario 20: Blue-Green Deployment - How to implement?", a: "Deploy both versions with different labels. Update service selector to switch traffic.\nUse Ingress for route control." },
        { q: "Scenario 21: ImagePullBackOff - Root cause?", a: "Check image name/tag, credentials, and registry reachability.\nUse imagePullSecrets if private registry is used." },
        { q: "Scenario 22: Secret Mount Path Error - Debugging?", a: "Check that volumeMount and volumes use the correct secret name.\nEnsure keys are mounted properly and permissions allow access." },
        { q: "Scenario 23: StatefulSet Pod Pending - Why?", a: "PVC not bound or affinity constraints.\nCheck 'kubectl describe pod' and matching PV/storageClass." },
        { q: "Scenario 24: Log Rotation - Best practice?", a: "Best practice: write logs to stdout/stderr.\nUse log agents (Fluentd) that handle rotation from /var/log/containers." },
        { q: "Scenario 25: Pod on Wrong Node - How to control?", a: "Use node affinity or taints/tolerations to restrict scheduling to suitable nodes." },
        { q: "Scenario 26: Namespace Access Restriction - RBAC approach?", a: "Use RBAC to bind devs to specific namespaces and prevent kube-system access." },
        { q: "Scenario 27: Pod Floods Network - Mitigation?", a: "Use NetworkPolicies or Cilium to limit egress.\nSet CPU/memory/net limits and monitor with Prometheus." },
        { q: "Scenario 28: Ingress Not Routing - Troubleshooting?", a: "Check controller status, Ingress resource, DNS resolution, and service backends." },
        { q: "Scenario 29: Resource Quotas in Namespaces - Purpose?", a: "Apply ResourceQuota and LimitRange to control pod and container usage in multi-tenant clusters." },
        { q: "Scenario 30: Debug Pod Without Tools - Solution?", a: "Use ephemeral containers:\nkubectl debug -it <pod> --image=busybox --target=<container>" },
        { q: "Scenario 31: Helm Rollback - How to execute?", a: "Use 'helm rollback <release> <revision>' after checking 'helm history'. Useful when a deployment breaks." },
        { q: "Scenario 32: Helm Values Ignored - Why?", a: "Ensure to use '-f values.yaml'. CLI overrides take precedence over values.yaml." },
        { q: "Scenario 33: HPA Custom Metrics Not Working - Fix?", a: "Use Prometheus Adapter and expose metrics via custom.metrics.k8s.io.\nValidate API group and metric presence." },
        { q: "Scenario 34: CI/CD Push Breaks Prod - Prevention?", a: "Use separate namespaces or manual approvals. Implement Helm dry-run and smoke tests in pipelines." },
        { q: "Scenario 35: Duplicate CronJobs - Prevention?", a: "Set 'concurrencyPolicy: Forbid' and check controller stability.\nAvoid overlapping executions." },
        { q: "Scenario 36: Cluster Backup - Strategy?", a: "Use Velero for full backup of workloads and volumes. For etcd-only, use etcdctl snapshot." },
        { q: "Scenario 37: Compromised ServiceAccount - Actions?", a: "Revoke token, delete secret, rotate credentials. Limit RBAC and audit access logs." },
        { q: "Scenario 38: CI/CD Deploys Wrong Image - Prevention?", a: "Avoid :latest tags. Use semantic or commit-based tagging and image digest for immutability." },
        { q: "Scenario 39: EKS/GKE Cost Optimization - Strategies?", a: "Use autoscaler, spot nodes, and monitor with tools like Kubecost. Shut down unused dev clusters." },
        { q: "Scenario 40: PVC Mounted But Fails - Troubleshooting?", a: "Check mount permissions, initContainers to chown, and user ID compatibility with file system." }
    ],
    "Important": [
        { q: "What are the types of Docker volumes?", a: "Docker provides three types of volumes:\n1. Host Volume – Maps a directory from the host machine to the container.\n2. Anonymous Volume – Docker manages the volume without a specific name.\n3. Named Volume – User-defined and persisted independently of containers." },
        { q: "What is Kubernetes taint and toleration?", a: "• Taint is applied on a node to restrict pod scheduling.\n• Toleration is applied on pods to allow them to run on tainted nodes.\nUsed to isolate workloads, e.g., run only specific apps on specific nodes." },
        { q: "If you want to add an existing resource to the Terraform state file?", a: "Use the command: terraform import <resource_address> <resource_id>" },
        { q: "Can pod-to-pod communication happen by default?", a: "Yes. In Kubernetes, all pods can communicate with each other by default within the cluster because the cluster network is flat and non-restrictive unless Network Policies restrict it." },
        { q: "What are Helm charts?", a: "A Helm chart is a package of YAML templates used to deploy Kubernetes applications through versioned, repeatable, and configurable deployments." },
        { q: "How will you deploy Jenkins in your organization?", a: "4 common approaches:\n1. EC2 installation (manual setup)\n2. Docker container\n3. Helm chart deployment on EKS\n4. Kubernetes YAML manifests" },
        { q: "When you deploy Jenkins with Helm, what is the folder structure?", a: "A Helm chart typically includes:\nChart.yaml\nvalues.yaml\ntemplates/\n  deployment.yaml\n  service.yaml\n  ingress.yaml\n  configmap.yaml\n  pvc.yaml\ncharts/\nREADME.md" },
        { q: "How much time will be taken for a Jenkins job completion?", a: "Depends on pipeline stages, application build time, tests, and infra speed. Typically ranges from 1 minute to 15+ minutes depending on the complexity." },
        { q: "Where do you deploy the microservices?", a: "Usually deployed in:\n• Kubernetes (EKS)\n• Docker containers\n• ECS\n• EC2\n• Fargate" },
        { q: "Who manages the infrastructure in your organization?", a: "Infrastructure is usually managed by the DevOps Team using IaC tools like Terraform, CloudFormation, and Ansible." },
        { q: "Application deployed in EKS but not accessible externally — how will you debug?", a: "Steps:\n1. Check Service type (LoadBalancer / NodePort).\n2. Check Ingress configuration.\n3. Check Security Groups inbound rules.\n4. Check NACLs / VPC routing.\n5. Check DNS mapping.\n6. Check if pods are running and service endpoints exist." },
        { q: "Explain the Git branching strategy.", a: "Most common: Gitflow\n• main/master → Stable production\n• develop → Active development\n• feature/ → New features\n• release/ → Pre-production\n• hotfix/ → Quick patches on production" },
        { q: "How can you restrict pod-to-pod communication?", a: "Using Kubernetes NetworkPolicies:\n• Deny all traffic\n• Allow only specific namespace/app/labels" },
        { q: "Suppose Jenkins pipeline fails — how will you debug?", a: "• Check Console output\n• Check agent availability\n• Validate credentials\n• Check Docker build errors\n• Check Git authentication\n• Check Stage-specific errors" },
        { q: "How does communication happen in an EKS cluster?", a: "Through:\n• Kubernetes network (CNI plugin)\n• Service (ClusterIP / NodePort / LoadBalancer)\n• CoreDNS\n• VPC routing" },
        { q: "Python program to read log file and print error count", a: "count = 0\nwith open(\"app.log\", \"r\") as f:\n    for line in f:\n        if \"ERROR\" in line:\n            count += 1\nprint(\"Total ERROR messages:\", count)" },
        { q: "What type of agents are you using in Jenkins?", a: "• Static EC2 agents\n• Dynamic agents via Kubernetes plugin\n• Docker agents\n• Self-hosted runners" },
        { q: "How are you deploying Jenkins: EC2 or EKS?", a: "Depends on the organization, but commonly using EKS with Helm for scalability." },
        { q: "Difference between StatefulSet and Deployment?", a: "Deployment:\n• Stateless apps\n• No stable identity\n• ReplicaPods identical\n\nStatefulSet:\n• Stateful apps (DB, Kafka)\n• Stable network identity\n• Persistent storage per pod" },
        { q: "Explain RBAC in Kubernetes.", a: "RBAC = Role-Based Access Control\nControls who can access what using:\n• Roles\n• ClusterRoles\n• RoleBinding\n• ClusterRoleBinding" },
        { q: "Jenkins deployed via Helm — how to update plugins?", a: "Two ways:\n1. Update values.yaml → plugin list\n2. Upgrade chart:\nhelm upgrade jenkins -f values.yaml jenkins/jenkins" },
        { q: "How do you store secrets in Kubernetes?", a: "Using:\n• Kubernetes Secrets (Base64 encoded)\n• AWS Secrets Manager + CSI driver\n• HashiCorp Vault" },
        { q: "Lost Jenkins password — how to restore?", a: "• If using Helm:\nkubectl exec -it <pod> -- cat /var/jenkins_home/secrets/initialAdminPassword\n• Reset via admin account\n• Restore backup (if taken)" },
        { q: "What plugins do you use in Jenkins?", a: "Common ones:\n• Git\n• Pipeline\n• Credentials\n• Blue Ocean\n• Docker\n• Kubernetes\n• Slack\n• SonarQube" },
        { q: "How many DevOps engineers are in your team?", a: "Typical answer: 4–6 DevOps engineers, depending on project size." },
        { q: "What is your project architecture?", a: "A clean high-level answer:\n• Microservices in EKS\n• CI/CD via Jenkins/GitHub Actions\n• IaC via Terraform\n• Monitoring via Prometheus & Grafana\n• Images in ECR\n• Logs in CloudWatch" },
        { q: "How do you receive tickets?", a: "Through:\n• JIRA\n• ServiceNow\n• Azure DevOps Boards" },
        { q: "EC2 instance type used to deploy apps? Is it sufficient?", a: "Example answer: t3.medium / t3.large\nChoose based on:\n• CPU\n• Memory\n• Auto-scaling\n• Application load" },
        { q: "Deployment strategies used?", a: "• Rolling update\n• Blue-Green\n• Canary\n• Recreate" },
        { q: "Install Nginx on 10 servers using Ansible?", a: "Use inventory + playbook:\nhosts: web\ntasks:\n  - name: Install nginx\n    apt: name=nginx state=present" },
        { q: "Explain Ansible structure.", a: "Folder structure:\ninventories/\nroles/\nplaybooks/\ngroup_vars/\nhost_vars/\nansible.cfg" },
        { q: "What is a playbook?", a: "A YAML file that defines tasks, modules, and roles to execute automation." },
        { q: "What rollback strategies do you follow?", a: "• Helm rollback\n• Kubernetes deployment revision\n• EC2 AMI rollback\n• Terraform rollback (manual state revert)\n• Git revert" },
        { q: "Difference between git clone vs git fork, merge vs rebase", a: "• Clone: Copy repo locally\n• Fork: Copy repo under your account\n• Merge: Combines changes with merge commit\n• Rebase: Rewrites commit history for cleaner log" },
        { q: "How to combine multiple commits into single commit?", a: "git rebase -i HEAD~n" },
        { q: "What is .git folder?", a: "It stores:\n• Repo history\n• Branches\n• Objects\n• Configuration" },
        { q: "If you lost .git folder — how to restore?", a: "You cannot fully restore. You must reinitialize:\ngit init\ngit remote add origin <url>\ngit fetch" },
        { q: "Difference between git pull and git fetch?", a: "• git fetch → downloads but doesn't merge\n• git pull → downloads + merges automatically" },
        { q: "Where do you store Jenkinsfiles and Dockerfiles?", a: "Typically in each service repository:\n/app\n  Dockerfile\n  Jenkinsfile" },
        { q: "Docker stops & restarts — data lost. How to fix?", a: "Use Docker volumes or bind mounts to persist data." },
        { q: "What is CrashLoopBackOff?", a: "Pod keeps crashing and Kubernetes keeps restarting it.\nCauses:\n• App errors\n• Wrong configs\n• Missing dependencies\n• Liveness probe failure" },
        { q: "How to delete unused Docker containers and images?", a: "docker system prune -a" },
        { q: "What type of Load Balancers have you used?", a: "• ALB\n• NLB\n• CLB\n\nIn Kubernetes:\n• Ingress Controller\n• Service LoadBalancer" },
        { q: "What are S3 storage classes?", a: "• Standard\n• Standard-IA\n• One Zone-IA\n• Glacier\n• Glacier Deep Archive\n• Intelligent-Tiering" },
        { q: "How to locate the path of a file?", a: "find / -name filename" },
        { q: "How to delete log files >50MB and older than 30 days?", a: "find /var/log -type f -size +50M -mtime +30 -delete" }
    ]
};
...
function renderTopic(topic) {
    ...
    if (filtered.length === 0) {
        ...
        msg.innerHTML = `No matches for "${escapeHtml(searchTerm)}" in ${escapeHtml(topic)}.`;
...
// ---------------------- Important Q&A (Static/Pre-loaded) ----------------------
// Add your important Q&A here or they will be loaded dynamically.
...
function showVideoPractice() {
        ...
        <section class="video-practice-page" aria-labelledby="videoPracticeTitle">
            ...
            <div class="video-practice-layout">
                <main class="video-practice-main" aria-label="Recorder workspace">
                    ...
                    <section class="video-practice-library" aria-labelledby="vpLibraryTitle">
                        <div class="video-practice-library-header">
                            <div>
                                <p class="eyebrow">Saved locally</p>
                                <h3 id="vpLibraryTitle">Previous recordings</h3>
                                <p>Videos are saved in this browser so you can replay and download them later.</p>
                            </div>
                            <span id="vpLibraryCount" class="video-privacy-badge">0 videos</span>
                        </div>
    ...
    loadVideoPracticeSavedRecordings();
}
...
function bindVideoPracticeEvents() {
    ...
    document.getElementById('vpSavedRecordings')?.addEventListener('click', handleVideoPracticeSavedListClick);
...
function finishVideoPracticeRecording() {
    ...
    saveVideoPracticeRecording(blob);
    setVideoPracticeStatus('Review recording', false);
    setVideoPracticeButtonState({ hasRecording: true });
    setVideoPracticeMessage('Recording ready. It is saved locally in Previous recordings.', 'success');
}
...
async function deleteVideoPracticeRecording() {
    if (videoPracticeState.currentRecordingId) {
        await deleteVideoPracticeSavedRecording(videoPracticeState.currentRecordingId);
        videoPracticeState.currentRecordingId = null;
    }
    clearVideoPracticeRecording();
    cleanupVideoPracticeStream();
    stopVideoPracticeAudioMeter();
    setVideoPracticeIdleState();
    setVideoPracticeMessage('Recording deleted.', 'info');
...
function openVideoPracticeDb() {
    return new Promise((resolve, reject) => {
        if (!window.indexedDB) {
            reject(new Error('IndexedDB is unavailable'));
            return;
        }
        const request = indexedDB.open(videoPracticeDbName, 1);
        request.onupgradeneeded = () => {
            const db = request.result;
            if (!db.objectStoreNames.contains(videoPracticeStoreName)) {
                db.createObjectStore(videoPracticeStoreName, { keyPath: 'id' });
            }
        };
        request.onsuccess = () => resolve(request.result);
        request.onerror = () => reject(request.error || new Error('Could not open recordings database'));
    });
}

async function withVideoPracticeStore(mode, callback) {
    const db = await openVideoPracticeDb();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(videoPracticeStoreName, mode);
        const store = transaction.objectStore(videoPracticeStoreName);
...
async function saveVideoPracticeRecording(blob) {
    const recordedAt = videoPracticeState.lastRecordedAt || new Date();
    const id = `vp-${recordedAt.getTime()}`;
    const record = {
        id,
        title: `Video Practice ${recordedAt.toLocaleString()}`,
        createdAt: recordedAt.toISOString(),
        duration: videoPracticeState.elapsedSeconds,
        mimeType: videoPracticeState.recordingMimeType || blob.type || 'video/webm',
        size: blob.size,
        blob
    };
    try {
        await withVideoPracticeStore('readwrite', store => store.put(record));
        videoPracticeState.currentRecordingId = id;
        await loadVideoPracticeSavedRecordings();
    } catch (error) {
        console.error('Could not save Video Practice recording:', error);
        setVideoPracticeMessage('Recording is ready, but browser storage is unavailable. Please download it now.', 'error');
    }
}

async function loadVideoPracticeSavedRecordings() {
    const list = document.getElementById('vpSavedRecordings');
    if (!list) return;
    revokeVideoPracticeSavedUrls();
    try {
        const records = await withVideoPracticeStore('readonly', store => store.getAll());
        videoPracticeState.savedRecordings = records
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
            .map(record => ({ ...record, url: URL.createObjectURL(record.blob) }));
        videoPracticeState.savedRecordingUrls = videoPracticeState.savedRecordings.map(record => record.url);
        renderVideoPracticeSavedRecordings();
    } catch (error) {
...
function renderVideoPracticeSavedRecordings() {
    const list = document.getElementById('vpSavedRecordings');
    const count = document.getElementById('vpLibraryCount');
    if (!list) return;
    if (count) count.textContent = `${videoPracticeState.savedRecordings.length} video${videoPracticeState.savedRecordings.length === 1 ? '' : 's'}`;
    if (!videoPracticeState.savedRecordings.length) {
        list.innerHTML = '<div class="video-practice-empty-library">No videos recorded yet. Record your first practice answer to see it here.</div>';
        return;
    }
    list.innerHTML = videoPracticeState.savedRecordings.map(record => `
        <article class="video-practice-saved-card" data-id="${escapeHtml(record.id)}">
            <video src="${record.url}" controls preload="metadata" playsinline></video>
            <div class="video-practice-saved-meta">
                <h4>${escapeHtml(record.title)}</h4>
                <p>${new Date(record.createdAt).toLocaleString()} • ${formatVideoPracticeDuration(record.duration || 0)} • ${formatVideoPracticeFileSize(record.size || 0)}</p>
                <div class="video-practice-saved-actions">
                    <button type="button" class="vp-mini-btn" data-saved-action="download" data-id="${escapeHtml(record.id)}">Download</button>
                    <button type="button" class="vp-mini-btn vp-mini-danger" data-saved-action="delete" data-id="${escapeHtml(record.id)}">Delete</button>
                </div>
            </div>
        </article>
    `).join('');
...
function handleVideoPracticeSavedListClick(event) {
    const button = event.target.closest('[data-saved-action]');
    if (!button) return;
    const record = videoPracticeState.savedRecordings.find(item => item.id === button.dataset.id);
    if (!record) return;
    if (button.dataset.savedAction === 'download') downloadVideoPracticeSavedRecording(record);
    if (button.dataset.savedAction === 'delete') deleteVideoPracticeSavedRecording(record.id);
}

function downloadVideoPracticeSavedRecording(record) {
    const link = document.createElement('a');
    link.href = record.url;
    link.download = `video-practice-${formatVideoPracticeTimestamp(new Date(record.createdAt))}.${record.mimeType?.includes('mp4') ? 'mp4' : 'webm'}`;
    link.click();
    setVideoPracticeMessage('Saved recording download started.', 'success');
}

async function deleteVideoPracticeSavedRecording(id) {
    const confirmed = window.confirm('Delete this saved recording? This action cannot be undone.');
    if (!confirmed) return;
    try {
        await withVideoPracticeStore('readwrite', store => store.delete(id));
        await loadVideoPracticeSavedRecordings();
        setVideoPracticeMessage('Saved recording deleted.', 'info');
    } catch (error) {
        console.error('Could not delete saved recording:', error);
        setVideoPracticeMessage('Could not delete the saved recording. Please try again.', 'error');
    }
}
